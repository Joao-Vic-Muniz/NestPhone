...
const historyData = [
  {
    ...
    color: "from-[var(--color-primary-dark)]/80"
  },
  {
  ...
  },
  {
    ...
    image: MotorolaV3,
    color: "from-[var(--color-primary)]/80"
  },
  {
    ...
    image: Iphone,
    color: "from-[var(--color-primary-light)]/80"
  },
  {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-transparent py-6 md:py-10">
      <div className="relative w-full max-w-7xl mx-auto px-4 md:px-24 flex items-center justify-center">
        ...
        <button 
          onClick={prevStep}
          className="hidden md:block absolute left-2 md:left-4 z-30 p-2 text-[var(--color-primary-light)] hover:scale-110 transition-transform active:scale-90"
        >
        ...
        <div className="w-full min-h-[650px] md:min-h-[550px] flex items-center overflow-hidden relative">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className={`flex flex-col md:flex-row items-center w-full bg-gradient-to-br ${historyData[index].color} to-[var(--color-bg-card)]/20 backdrop-blur-md border border-white/10 rounded-[2.5rem] md:rounded-[3.5rem] p-8 md:p-16 gap-8 md:gap-12 shadow-2xl`}
            >
              ...
              <div className="w-full md:w-1/2 text-center md:text-left space-y-4 md:space-y-6">
                <motion.h2 
                  ...
                  className="font-title text-2xl md:text-5xl text-[var(--color-action)] leading-tight"
                >
                ...
                <motion.p 
                  ...
                  className="text-[var(--color-text-main)] font-text text-base md:text-xl leading-relaxed"
                >
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-8 pt-4">
                   <div className="flex gap-2">
                      {historyData.map((_, i) => (
                        <button
                          key={i}
                          onClick={() => setIndex(i)}
                          className={`h-2 rounded-full transition-all duration-500 ${i === index ? 'w-10 bg-[var(--color-primary-light)]' : 'w-3 bg-[var(--color-primary-dark)]/50 hover:bg-[var(--color-primary)]'}`} 
                        />
                      ))}
                   </div>
                   ...
                   <div className="flex md:hidden gap-10">
                      <button 
                        onClick={prevStep} 
                        className="p-3 text-[var(--color-primary-light)] border border-[var(--color-primary)]/30 rounded-full active:bg-[var(--color-primary)]/20"
                      >
                        <ChevronLeft size={28} />
                      </button>
                      <button 
        ...
        <button 
          onClick={nextStep}
          className="hidden md:block absolute right-2 md:right-4 z-30 p-2 text-[var(--color-primary-light)] hover:scale-110 transition-transform active:scale-90"
        >
